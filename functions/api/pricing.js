...
const DATABASE_CARD_LIST = Array.from(DATABASE_CARDS);
...
const NORMALIZED_DATABASE_CARD_MAP = buildNormalizedCardMap(DATABASE_CARD_LIST);
const DATABASE_ENERGY_CARDS = DATABASE_CARD_LIST.filter(card => card.toLowerCase().includes('energy'));
...
function parseCleanedPriceData(cleanedRecords, setAbbr) {
  ...
  for (const record of cleanedRecords) {
    ...
    const finalCardKey = DATABASE_CARDS.has(cardKey)
      ? cardKey
      : NORMALIZED_DATABASE_CARD_MAP.get(normalizedCardKey);
