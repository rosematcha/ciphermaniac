...
import { buildTrendReport, buildCardTrendReport } from '../functions/lib/onlineMeta.ts';
...
describe('Trends Logic', () => {
    describe('buildTrendReport', () => {
        it('should filter out small tournaments (< 16 players)', () => {
            ...
            const decks = [
                createDeck('t1', 'Deck A'),
                createDeck('t2', 'Deck A'),
                createDeck('t3', 'Deck A')
            ];
            ...
            assert.strictEqual(report.tournaments.find(t => t.id === 't2'), undefined);

            // Deck A should only have 2 appearances tracked
            const series = report.series.find(s => s.displayName === 'Deck A');
        ...
        it('should backfill missing tournaments with 0 share', () => {
            const tournaments = [
                createTournament('t1', '2023-01-01', 20, 20),
                createTournament('t2', '2023-01-02', 20, 20)
            ];
            ...
            const t1Entry = series.timeline.find(t => t.tournamentId === 't1');
            const t2Entry = series.timeline.find(t => t.tournamentId === 't2');
    ...
    describe('buildCardTrendReport', () => {
        it('should exclude cards with 0% current share from Rising list', () => {
            ...
            const decks = [
                { ...createDeck('t1', 'Deck'), cards: [{ name: 'Card A', count: 1 }] }
            ];

            const report = buildCardTrendReport(decks, tournaments, { minAppearances: 1, topCount: 5 });

            const risingNames = report.rising.map(c => c.name);
