 ...
 * @returns
 */
export function toggleFiltersPanel({ focusFirstControlOnOpen = false, focusSelector = DEFAULT_FOCUS_SELECTOR, restoreFocusOnClose = false } = {}) {
 ...
 * @returns
 */
export function initFiltersToggle({ owner = 'ui', focusFirstControlOnOpen = true, restoreFocusOnClose = true, focusSelector = DEFAULT_FOCUS_SELECTOR } = {}) {
    const { panel, toggle } = getElements();
    if (!panel || !toggle) {
        return;
    }
