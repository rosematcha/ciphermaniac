...
function deriveSuccessTags(deck, sizes = null, counts = null) {
    ...
    let players = Number.isFinite(deck?.tournamentPlayers) && deck.tournamentPlayers !== null
...
export function filterDecksBySuccess(decks, tag) {
    ...
    (Array.isArray(decks) ? decks : []).forEach(deck => {
        ...
        const players = Number.isFinite(deck.tournamentPlayers) && deck.tournamentPlayers !== null
            ? Number(deck.tournamentPlayers)
            : Number(deck.players);
 ...
 */
export function generateFilteredReport(decks, archetypeBase, includeId, excludeId, includeOperator = null, includeCount = null) {
    const filters = [];
    if (includeId) {
        filters.push({
            cardId: includeId,
