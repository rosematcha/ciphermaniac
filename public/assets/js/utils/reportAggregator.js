...
function buildAggregationKey(item) {
    ...
    const numberPart = typeof item.number === 'string' || typeof item.number === 'number' ? String(item.number).toUpperCase() : '';
...
export function aggregateReports(reports) {
    ...
    const aggregatedItems = finalizeAggregates(aggregates, totalDecks).sort((left, right) => (right.found ?? 0) - (left.found ?? 0));
