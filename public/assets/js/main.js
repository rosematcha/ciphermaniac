...
import { fetchArchetypeReport, fetchArchetypesList, fetchLimitlessTournaments, fetchReport, fetchTournamentsList } from './api.js';
...
import { closeFiltersPanel as closeFiltersPanelState, openFiltersPanel as openFiltersPanelState, toggleFiltersPanel as toggleFiltersPanelState } from './utils/filtersPanel.js';
...
function setupDropdownFilters(state) {
    ...
    const onDocumentPointerDown = (event) => {
        ...
        const dropdownList = Object.values(dropdowns).filter((d) => !!d);
...
async function applyCurrentFilters(state) {
    ...
    const existingSets = Array.isArray(state.selectedSets) && state.selectedSets.length > 0 ? [...state.selectedSets] : readSelectedSets();
...
async function initializeTournamentSelector(state) {
    ...
    const tournaments = await safeAsync(() => fetchTournamentsList(), 'fetching tournaments list', ['2025-08-15, World Championships 2025'] // fallback
    );
    // Check if online meta report exists separately (not in tournaments.json)
    const hasOnlineMeta = await safeAsync(async () => {
        const response = await fetch(`${CONFIG.API.R2_BASE}/reports/${encodeURIComponent(DEFAULT_ONLINE_META)}/master.json`, {
            method: 'HEAD'
        });
        return response.ok;
    }, 'checking availability of online meta report', false);
...
async function hydrateOnlineTournaments(state, options = {}) {
    const tournaments = await safeAsync(() => fetchLimitlessTournaments(options), 'fetching Limitless online tournaments', []);
...
function setupControlHandlers(state) {
    ...
    const handleSearch = debounce(async () => {
        await applyFiltersSort(state.current.items, state.overrides);
        setStateInURL({ query: elements.search.value }, { merge: true, replace: true });
    });
    ...
    const urlState = getStateFromURL();
    if (urlState.query) {
        elements.search.value = urlState.query;
    }
    if (urlState.sort) {
