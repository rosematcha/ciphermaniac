...
function buildTooltip(card, options) {
    ...
    if (options.mode === 'archetype' && options.archetype) {
        const usage = card.usageByArchetype.find(entry => entry.archetype === options.archetype);
        if (usage) {
            lines.push(`Primary usage: ${formatPercent(usage.ratio)} in ${usage.displayName} ` +
...
function renderBinderSections() {
    ...
    if (!state.binderData || state.isBinderDirty || state.binderData.meta.totalDecks === 0) {
        const prompt = state.isBinderDirty && state.binderData
...
function updateStats() {
    ...
    const missingText = metrics && metrics.missingPrices
...
async function computeBinderMetrics(binderData, context) {
    ...
    const priceEntries = await Promise.all(Array.from(unique.values()).map(async (entry) => {
    ...
    try {
        ...
        if (pendingArchetypeSelection) {
            ...
            if (nextSelectedArchetypes.size === 0) {
                ...
                nextSelectedArchetypes = new Set(Array.from(nextSelectedArchetypes).filter(archetype => availableArchetypes.has(archetype)));
                if (!nextSelectedArchetypes.size) {
                    nextSelectedArchetypes = new Set(availableArchetypes);
                }
            }
...
function bindControlEvents() {
    ...
    if (elements.archetypeSearch) {
        elements.archetypeSearch.addEventListener('input', debounce(event => {
...
async function handleImportLayout(event) {
    ...
    try {
        ...
        if (validTournaments.length === 0) {
            ...
            alert('None of the tournaments in this export are currently available. Please ensure the same tournaments are loaded.');
            return;
        }
...
async function initialize() {
    try {
        ...
        if (!state.tournaments.length) {
            showError('No tournaments are available. This may be a temporary network issue. Please refresh the page to try again.');
            return;
        }
        const savedSelections = loadSelections();
        if (savedSelections && savedSelections.tournaments.length) {
            state.selectedTournaments = new Set(savedSelections.tournaments.filter(tournament => state.tournaments.includes(tournament)));
