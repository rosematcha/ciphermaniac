...
function inferTrainerSubtype(card) {
    ...
    if (TRAINER_STADIUM_KEYWORDS.some(keyword => name.includes(keyword)) ||
        TRAINER_STADIUM_KEYWORDS.some(keyword => uid.includes(keyword))) {
        return 'stadium';
    }
    if (TRAINER_TOOL_KEYWORDS.some(keyword => name.includes(keyword)) ||
        TRAINER_TOOL_KEYWORDS.some(keyword => uid.includes(keyword))) {
        return 'tool';
    }
    if (TRAINER_SUPPORTER_KEYWORDS.some(keyword => name.includes(keyword)) ||
        TRAINER_SUPPORTER_KEYWORDS.some(keyword => uid.includes(keyword))) {
        return 'supporter';
    }
    if (TRAINER_ITEM_KEYWORDS.some(keyword => name.includes(keyword)) ||
...
function buildTcgliveExportString(entries) {
    ...
    entries.forEach(entry => {
        ...
        const key = entry.primaryCategory === 'trainer' ? 'trainer' : entry.primaryCategory === 'energy' ? 'energy' : 'pokemon';
...
async function handleSkeletonExport(event) {
    ...
    try {
        ...
        const baseMessage = method === 'prompt' ? 'Deck list ready in a prompt for manual copy.' : 'Copied TCG Live deck list to clipboard.';
...
let lastRenderedThreshold = null;
 ...
 */
function renderCardsWithThreshold(threshold) {
...
function loadFilterCombination(filters) {
    ...
    const promise = (async () => {
        try {
            ...
            const { fetchAllDecks, generateReportForFilters, filterDecksBySuccess } = await import('./utils/clientSideFiltering.js');
...
async function applyFilters() {
    ...
    for (const filter of activeFilters) {
        ...
        if (info?.alwaysIncluded && (!filter.operator || filter.operator === '')) {
            updateFilterMessage(`${info.name} is in 100% of decks. Select "Any" or a quantity operator to filter by copy count.`, 'info');
            return;
        }
    }
...
async function loadSuccessBaseline() {
    ...
    const { fetchAllDecks, filterDecksBySuccess, generateReportForFilters } = await import('./utils/clientSideFiltering.js');
    const allDecks = await fetchAllDecks(state.tournament);
