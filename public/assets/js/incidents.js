...
export async function initIncidentsPage(options = {}) {
    const { listSelector = '#incidentsList', paginationSelector = '#incidentsPagination', fetchTournaments = fetchTournamentsList } = options;
    const listContainer = document.querySelector(listSelector);
    const paginationContainer = paginationSelector ? document.querySelector(paginationSelector) : null;
    if (!listContainer) {
...
function renderPaginationControls(container, currentPage, totalItems, perPage, onPageChange) {
    if (!container) {
        return;
    }
