...
export function renderChart(container, points) {
    ...
    function onHitMove(event) {
        try {
            ...
            if (point) {
                showGraphTooltip(`<strong>${escapeHtml(prettyTournamentName(point.tournament))}</strong><div>${(point.pct || 0).toFixed(1)}%</div>`, event.clientX, event.clientY);
            }
        }
        catch {
    ...
    points.forEach((point, index) => {
        ...
        hitDot.addEventListener('mousemove', (event) => showGraphTooltip(`<strong>${escapeHtml(prettyTournamentName(point.tournament))}</strong><div>${(point.pct || 0).toFixed(1)}%</div>`, event.clientX, event.clientY));
        hitDot.addEventListener('mouseenter', (event) => showGraphTooltip(`<strong>${escapeHtml(prettyTournamentName(point.tournament))}</strong><div>${(point.pct || 0).toFixed(1)}%</div>`, event.clientX, event.clientY));
...
export function renderCopiesHistogram(container, overall) {
    ...
    const maxPercentage = Math.max(1, ...distribution.map(distributionItem => {
    ...
    for (const copies of copiesToShow) {
        ...
        column.addEventListener('mousemove', (event) => showGraphTooltip(escapeHtml(tooltipText), event.clientX, event.clientY));
        column.addEventListener('mouseenter', (event) => showGraphTooltip(escapeHtml(tooltipText), event.clientX, event.clientY));
 ...
 * @param container - Container element for the events table
 * @param rows - Array of tournament data rows
 */
    ...
    rows.forEach(rowData => {
        ...
        cellValues.forEach((value, index) => {
            ...
            if (value instanceof HTMLElement) {
                ...
                tableCell.textContent = String(value);
            }
