...
const elements = {
    ...
    minSlider: document.getElementById('trend-min-tournaments'),
...
const state = {
    trendData: 
    /** @type {null|{ series: any[], tournaments: any[], generatedAt?: string, minAppearances?: number, windowStart?: string|null, windowEnd?: string|null }} */ (null),
    cardTrends: null,
    suggestions: null,
    isLoading: false,
    isHydrating: false,
...
function buildMetaLines(trendData, topN = 5) {
    ...
    const timelineDates = Array.from(new Set(seriesWithBins.flatMap(entry => entry.daily.map(pt => String(pt.date || ''))))).filter(Boolean);
    ...
    const lines = seriesWithBins.map(entry => {
        ...
        const startAvg = points.slice(0, Math.max(1, Math.ceil(points.length * 0.3))).reduce((a, b) => a + b, 0) /
            Math.max(1, Math.ceil(points.length * 0.3));
        const recentAvg = points.slice(-Math.max(1, Math.ceil(points.length * 0.3))).reduce((a, b) => a + b, 0) /
            Math.max(1, Math.ceil(points.length * 0.3));
...
function renderCardMovers(suggestions, cardTrends) {
    ...
    const risingList = (suggestions?.onTheRise && suggestions.onTheRise.length && suggestions.onTheRise) || cardTrends?.rising || [];
    const fallingList = (suggestions?.choppedAndWashed && suggestions.choppedAndWashed.length && suggestions.choppedAndWashed) ||
        cardTrends?.falling ||
        [];
    ...
    const normalizeCard = item => {
        const latest = item.recentAvg ?? item.latest ?? item.currentShare ?? item.endShare ?? item.startShare ?? item.avgShare ?? 0;
...
function renderMetaChart() {
    ...
    const lines = Array.from(elements.metaChart.querySelectorAll('.meta-line'));
    const hitlines = Array.from(elements.metaChart.querySelectorAll('.meta-line-hit'));
    const legendItems = elements.legend
        ? Array.from(elements.legend.querySelectorAll('.legend-item'))
        : [];
...
async function hydrateFromDecks() {
    ...
    try {
        ...
        const tournaments = state.trendData.tournaments && state.trendData.tournaments.length
