...
class SocialGraphicsGenerator {
    ...
    async generateGraphics() {
        ...
        try {
            ...
            this.currentTournamentData = await fetchReportResource(`${tournamentFolder}/master.json`, 'tournament data', 'object', 'tournament data');
            // Load comparison tournament data if selected
            if (comparisonTournamentFolder && comparisonTournamentFolder !== tournamentFolder) {
                try {
                    this.comparisonTournamentData = await fetchReportResource(`${comparisonTournamentFolder}/master.json`, 'comparison tournament data', 'object', 'comparison tournament data');
                }
                catch (error) {
            ...
            if (currentIndex < this.tournaments.length - 1) {
                ...
                try {
                    this.previousTournamentData = await fetchReportResource(`${previousFolder}/master.json`, 'previous tournament data', 'object', 'previous tournament data');
                }
                catch (error) {
        ...
        return new Promise(resolve => {
            ...
            const originalOnload = () => {
                try {
                    ...
                    const canvasWidth = _cardSize === 'featured'
                        ? 400
                        : _cardSize === 'medium'
                            ? 300
                            : _cardSize === 'small'
                                ...
                                : _cardSize === 'tiny'
                                    ? 200
                                    : 300;
                    const canvasHeight = _cardSize === 'featured'
                        ? 353
                        : _cardSize === 'medium'
                            ? 160
                            : _cardSize === 'small'
                    ...
                    if (img.naturalWidth > 0 && img.naturalHeight > 0) {
                        ...
                        ctx.drawImage(img, cropParams.left, cropParams.top, sourceWidth, sourceHeight, drawX, drawY, drawWidth, drawHeight);
                        // eslint-disable-next-line no-param-reassign
