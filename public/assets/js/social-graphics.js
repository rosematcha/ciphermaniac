...
class SocialGraphicsGenerator {
  ...
  populateTournamentSelect() {
    ...
    this.tournaments.forEach((tournament) => {
      ...
      const nameWithoutDate = tournament.name.replace(
        /^\d{4}-\d{2}-\d{2}, /,
        "",
  ...
  async loadConsistentLeaders() {
    try {
      const suggestions = await fetchReportResource(
        "suggestions.json",
        "suggestions",
        "object",
        "suggestions",
      ...
      const consistentLeadersCategory = suggestions.categories.find(
        (cat) => cat.id === "consistent-leaders",
      );
  ...
  setupEventListeners() {
    document
      ...
      .addEventListener("click", () => this.generateGraphics());
    document
      .getElementById("export-png")
      .addEventListener("click", () => this.exportGraphics("png"));
    document
      .getElementById("export-jpg")
      .addEventListener("click", () => this.exportGraphics("jpg"));
  }

  async generateGraphics() {
    const tournamentFolder = this.getFieldValue("tournament-select");
    const comparisonTournamentFolder = this.getFieldValue(
      "comparison-tournament-select",
    ...
    try {
      ...
      if (
        comparisonTournamentFolder &&
        comparisonTournamentFolder !== tournamentFolder
      ) {
        try {
      ...
      const currentIndex = this.tournaments.findIndex(
        (tournament) => tournament.folder === tournamentFolder,
      );
  ...
  async renderGraphics() {
    ...
    if (filteredData.length > 0) {
      const featuredCard = await this.createCardGraphic(
        filteredData[0],
        1,
        displayMode,
        "featured",
    ...
    if (filteredData.length > 1) {
      ...
      for (let i = 1; i <= 3 && i < filteredData.length; i++) {
        const mediumCard = await this.createCardGraphic(
          filteredData[i],
          i + 1,
          displayMode,
          "medium",
    ...
    if (filteredData.length > 4) {
      ...
      for (let i = 4; i <= 7 && i < filteredData.length; i++) {
        const smallCard = await this.createCardGraphic(
          filteredData[i],
          i + 1,
          displayMode,
          "small",
    ...
    if (filteredData.length > 8) {
      ...
      for (let i = 8; i <= 13 && i < filteredData.length; i++) {
        const bottomCard = await this.createCardGraphic(
          filteredData[i],
          i + 1,
          displayMode,
          "tiny",
    ...
    if (filteredData.length > 14) {
      ...
      for (let i = 14; i <= 19 && i < filteredData.length; i++) {
        const secondBottomCard = await this.createCardGraphic(
          filteredData[i],
          i + 1,
          displayMode,
          "tiny",
  ...
  createHeaderSection(displayMode) {
    ...
    const currentTournament = this.tournaments.find(
      (t) => t.folder === tournamentFolder,
    );
    const comparisonTournament = this.tournaments.find(
      (t) => t.folder === comparisonTournamentFolder,

    const currentName = currentTournament
      ? currentTournament.name.replace(/^\d{4}-\d{2}-\d{2}, /, "")
      : "";
    const comparisonName = comparisonTournament
      ? comparisonTournament.name.replace(/^\d{4}-\d{2}-\d{2}, /, "")
      : "";
  ...
  getFilteredData(displayMode) {
    const data = this.currentTournamentData.items.filter(
      (card) => card.set !== "SVE",
    );
  ...
  getRisingCards(currentData) {
    ...
    const comparisonData =
      this.comparisonTournamentData || this.previousTournamentData;
    ...
    return risingCards.sort(
      (first, second) => second.increase - first.increase,
    );
  }
  ...
  async createCardGraphic(
    ...
    if (imagePath) {
      ...
      console.warn(
        `No image found for ${card.name} (${card.set} ${card.number})`,
      );
    }
      ...
      const originalOnload = () => {
        try {
          ...
          const sourceWidth = Math.max(
            1,
            img.naturalWidth - cropParams.left - cropParams.right,
          );
          const sourceHeight = Math.max(
            1,
            img.naturalHeight - cropParams.top - cropParams.bottom,
          );
          ...
          if (img.naturalWidth > 0 && img.naturalHeight > 0) {
            ...
            let drawWidth; let drawHeight; let drawX; let 
drawY;
