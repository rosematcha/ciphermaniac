...
test('only Standard claims card previews', () => {
  assert.deepEqual(
...
test('scraped archetypes map onto the index shape the board renders', async () => {
  ...
  for (const entry of entries) {
    ...
    // No decklists behind a scraped format, so these are honestly empty rather
    // than zero-filled — the board reads `icons`, never `thumbnails`.
    assert.equal(entry.deckCount, null);
    assert.deepEqual(entry.thumbnails, []);
...
test('shares are percentages above the floor, not fractions', () => {
  for (const format of scraped) {
    for (const archetype of format.archetypes) {
      assert.ok(
        archetype.share > snapshot.shareFloor && archetype.share <= 100,
