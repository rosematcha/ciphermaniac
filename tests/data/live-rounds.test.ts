...
const reader: RoundReader = (slug, round) => {
...
test('every posted round is read, oldest first', async () => {
  const loaded = await fetchPostedRounds('test-2027', 3, reader);
  assert.deepEqual(
...
test('a second read costs one request, not the whole run', async () => {
  await fetchPostedRounds('test-2027', 3, reader);
  read = [];
  const loaded = await fetchPostedRounds('test-2027', 3, reader);
...
test('the round the event moves on from is archived on the pass that leaves it', async () => {
  await fetchPostedRounds('test-2027', 3, reader);
  read = [];
  await fetchPostedRounds('test-2027', 4, reader);
...
test('a round file that is not there yet is not remembered as absent', async () => {
  missing.add(2);
  const first = await fetchPostedRounds('test-2027', 3, reader);
  assert.equal(first[1], null);
  missing.clear();
  read = [];
  const second = await fetchPostedRounds('test-2027', 3, reader);
...
test('a failed read is not held either', async () => {
  ...
  await assert.rejects(fetchPostedRounds('test-2027', 2, flaky));
  fail = false;
  read = [];
  const loaded = await fetchPostedRounds('test-2027', 2, flaky);
  assert.equal(loaded[0]?.round, 1);
  assert.deepEqual(read, ['test-2027|1', 'test-2027|2']);
});

test('each event keeps its own archive', async () => {
  await fetchPostedRounds('test-2027', 2, reader);
  read = [];
  await fetchPostedRounds('other-2027', 2, reader);
