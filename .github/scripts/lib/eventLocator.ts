...
import { POKEDATA_SITE, type PokedataPull } from './pokedata.ts';
...
export interface Publisher {
  /** The published index, or null when none exists yet. Throws on a transport failure. */
  readIndex(): Promise<LocatorIndex | null>;
...
export async function runEventLocator(options: RunOptions): Promise<RunResult> {
  ...
  const previous = await publisher.readIndex();
...
export function createLocalPublisher(root: string): Publisher {
  ...
  return {
    async readIndex() {
      try {
        return JSON.parse(await readFile(pathFor(LOCATOR_INDEX_KEY), 'utf8')) as LocatorIndex;
      } catch (error) {
...
export function createR2Publisher(config: R2Config): Publisher {
  ...
  return {
    async readIndex() {
      const result = await getJsonResult<LocatorIndex>(client, config.bucket, LOCATOR_INDEX_KEY);
      ...
      throw new Error(`Could not read the published index (${result.status})`, { cause: result.error });
    },
    write: (key, value) =>
