...
import { archetypeKey, archetypeSlug, type NormalizedEvent } from '../../shared/data/contracts.ts';
...
function legacyEventToSource(
  ...
  for (const deck of decks) {
    ...
    if (deck.archetype) {
      archetypeByTp.set(String(deck.playerId), deck.archetype);
    }
  ...
  return {
    ...
    standings: players.map(p => ({
      ...
      opw: p.opw ?? null,
      oopw: p.oopw ?? null,
      madePhase2: p.madePhase2,
      madeTopCut: p.madeTopCut,
      decklistPublished: p.decklistPublished,
      dropped: p.dropped,
    ...
    matches: matches.map(m => ({
      ...
      p1Id: m.player1Id!,
      p2Id: m.player2Id ?? null,
      winner: m.winnerCode ?? null
    }))
  };
}
...
function onlineToNormalized(decks: LegacyDeck[], synonymDb: SynonymDatabase | null): NormalizedEvent {
  ...
  const entries = withList.map((deck, i) => {
    ...
    const cards = [...aggregateCanonicalCardsPerDeck(deck.cards ?? [], synonymDb).values()].map(c => ({
      canonical: { uid: c.uid, name: c.name, set: c.set, number: c.number },
      printings: [],
      count: c.copies,
    ...
    return {
      participantId,
      deckId,
      playerRef: deck.playerId ?? null,
      name: deck.player ?? `deck-${i}`,
      placement: deck.placement ?? null,
      key,
      displayName: deck.archetype ?? 'Unknown',
      cards
    };
  });
  const participants = entries.map(e => ({
    ...
    opwPct: null,
    oopwPct: null,
    flags: { madePhase2: false, madeTopCut: false, dropped: false, dqed: false, late: false, decklistPublished: true },
    deckId: e.deckId
  }));
  const normDecks = entries
    .map(e => ({
      schemaVersion: 1 as const,
      deckId: e.deckId,
      participantId: e.participantId,
      playerRef: e.playerRef,
      archetype: { key: e.key, displayName: e.displayName, slug: archetypeSlug(e.key) },
      cards: e.cards,
      hasDecklist: true,
      successTags: []
    }))
  ...
  return {
    ...
    kind: 'online-window',
    meta: {
      name: 'Online - Last 14 Days',
      date: '1970-01-01',
      playerCount: deckTotal,
      format: null,
      division: null,
      hasDay2: false
    },
    ...
    matches: [],
    sourceRevisions: [
      { source: 'limitless-online', entityId: 'w14', sourceHash: sha256Hex(withList.length), fetchedAt: '' }
    ]
  };
}
...
async function main(): Promise<void> {
  ...
  const listed = await client.send(
    new ListObjectsV2Command({ Bucket: bucket, Prefix: 'reports/2026', Delimiter: '/' })
  );
  ...
  const publishEvent = async (event: NormalizedEvent, scopeKey: string): Promise<string> => {
    ...
    const nodeKey = computeNodeKey(
      {
        contractVersion: 1,
        builderVersion: 'event-artifacts-v1',
    ...
    candidates.forEach(c => {
      publishedKeys.push(c.key);
      totalBytes += c.body.length;
    });
  ...
  for (const base of eventBases) {
    const [decks, players, matches, meta, legacyMaster] = await Promise.all([
      load<LegacyDeck[]>(`${base}/decks.json`),
      load<LegacyPlayer[]>(`${base}/players.json`),
      load<LegacyMatch[]>(`${base}/matches.json`),
    ...
    if (!decks || !players || !meta) {
      console.log(`[shadow-all] skip ${base} (missing artifacts)`);
      continue;
    }
    const labsCode = (meta.labsCode as string) ?? base.replace(/[^a-z0-9]/gi, '').slice(-8);
    const event = labsSourceToNormalized(legacyEventToSource(labsCode, meta, decks, players, matches ?? []), {
      synonymDb: synonyms
    });
    ...
    if (legacyMaster) {
      const built = buildEventArtifacts(event, { synonymDb: synonyms }).get('master.json') as {
        items: { name: string; found: number }[];
      };
      const nextFound = new Map(built.items.map(i => [i.name, i.found]));
      const mismatches = legacyMaster.items.filter(
        i => nextFound.get(i.name) !== undefined && nextFound.get(i.name) !== i.found
      ).length;
      if (mismatches > 0) {
        parityFailures.push(`${base}: ${mismatches} master card-count mismatches`);
      }
    }
  ...
  const roots: Record<ReleaseScope, string> = {
    ...
    prices: `/${onlineRoot}`,
    catalogs: `/${onlineRoot}`,
    snapshots: `/${onlineRoot}`
  };
  const served: Record<ReleaseScope, string[]> = {
    online: ['master.json'],
    trends: [],
    players: [],
    prices: [],
  ...
  const manifest = composeRelease({
    releaseId: `shadow-${sha256HexString(JSON.stringify(eventRoots)).slice(0, 10)}`,
  ...
  console.log(
    `  parity           : ${parityFailures.length === 0 ? 'PASS (all scopes match legacy card counts)' : parityFailures.join('; ')}`
  );
