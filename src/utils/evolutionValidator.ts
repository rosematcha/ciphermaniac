...
export const BABY_POKEMON = new Set([
    'Azurill',
    'Budew',
    'Mantyke',
    'Smoochum',
...
export const EVOLUTION_EXCEPTIONS = new Set([
...
export const EVOLUTION_BYPASS_CARDS = new Set([
...
interface ValidationWarning {
    ...
    message: string;
    cardName: string;
    missingPreEvolutions?: string[];
    expectedPreEvolutions?: string[];
    conflictingEvolution?: string;
}
...
interface ValidationSummary {
    ...
    byType: Record<string, ValidationWarning[]>;
}
 ...
 * @returns Base Pokemon name
 */
export function getBasePokemonName(cardName: string): string {
    // Remove ex suffix
 ...
 * @param cardName - Full card name
 * @returns Trainer name or null if not a named variant
 */
export function getTrainerName(cardName: string): string | null {
 ...
 * @returns True if card is ex Pokemon
 */
export function isExPokemon(cardName: string): boolean {
 ...
 * @param cardName - Card name to check
 * @returns True if card is baby Pokemon
 */
export function isBabyPokemon(cardName: string): boolean {
 ...
 * @param pokemonName - Target Pokemon name
 * @returns Array of Pokemon that can evolve into target
 */
export function getPreEvolutions(pokemonName: string): string[] {
 ...
 * @param pokemonName - Base Pokemon name
 * @returns Array of Pokemon this can evolve into
 */
export function getEvolutions(pokemonName: string): string[] {
    const baseName = getBasePokemonName(pokemonName);
 ...
 * @param evolution - Evolution card name
 * @param basic - Basic card name
 * @returns True if named variants are compatible
 */
export function areNamedVariantsCompatible(evolution: string, basic: string): boolean {
 ...
 * @param deck - Map of card name -> {card, count}
 * @returns Array of validation warnings
 */
export function validateEvolutionRequirements(deck: Map<string, DeckEntry>): ValidationWarning[] {
    const warnings: ValidationWarning[] = [];
    const deckCards = new Map<string, { count: number; isEx: boolean; trainerName: string | null; originalNames: Set<string> }>(); // Pokemon name -> {count, isEx, trainerName}
    const hasSlowkingBypass = new Map<string, boolean>(); // Track Slowking bypass
    ...
    for (const [cardKey, entry] of deckCards) {
        ...
        if (baseName === 'Gardevoir') {
            ...
            try {
                (_logger.debug as any) &&
                    _logger.debug('DEBUGGING GARDEVOIR:', {
                        cardKey,
                        baseName,
                        isEx: entry.isEx,
        ...
        const compatiblePreEvolutions: string[] = [];
        ...
        if (!hasPreEvolution) {
            ...
            if (entry.isEx) {
                warnings.push({
                    ...
                    severity: 'warning',
                    message:
                        `${cardDisplayNames}: ex Pokemon without required pre-evolution. ` +
                ...
                warnings.push({
                    ...
                    severity: 'error',
                    message:
                        `${cardDisplayNames}: Evolution Pokemon played without smaller stage. ` +
                        `Needs one of: ${preEvolutions.join(', ')}.`,
                    cardName: cardKey,
                    missingPreEvolutions: preEvolutions
                });
            ...
            for (const evolution of evolutions) {
                ...
                if (deckCards.has(evolutionKey)) {
                    ...
                    warnings.push({
                        ...
                        severity: 'warning',
                        message:
                            `${cardDisplayNames}: ex Pokemon cannot evolve. Remove ${evolution} ` +
 ...
 * @returns True if card is a Pokemon
 */
function isPokemonCard(card: any): boolean {
 ...
 * @returns Summary object with counts and categorized warnings
 */
export function getEvolutionValidationSummary(warnings: ValidationWarning[]): ValidationSummary {
 ...
 * @param warnings - Validation warnings
 * @returns Formatted warnings text
 */
export function formatEvolutionWarnings(warnings: ValidationWarning[]): string {
