...
export interface ProgressController {
    ...
    updateStep: (index: number, status: 'pending' | 'loading' | 'complete' | 'error' | 'warning', details?: string) => void;
...
export async function loadWithCache<T, R>(
    ...
    const allResults = [...results, ...(uncachedResults.filter(resultItem => resultItem !== null) as { item: T; result: R; fromCache: boolean }[])];
