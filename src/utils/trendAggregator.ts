...
export function buildTrendDataset(
  ...
  const tournamentsWithTotals = sortedTournaments.map(tournament => ({
    id: tournament.id,
    name: tournament.name,
    date: tournament.date,
    deckTotal: tournamentIndex.get(tournament.id)?.deckTotal || 0
  }));
 ...
 * @param options
 */
export function buildCardTrendDataset(
  decks: Deck[],
  tournaments: Tournament[],
  options: CardTrendDatasetOptions = {}
): CardTrendDataset {
  const now = options.now ? new Date(options.now) : new Date();
