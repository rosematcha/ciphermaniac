...
export function renderChart(container: HTMLElement, points: ChartPoint[]): void {
    ...
    const scaleX = (index: number) => padding + (index * (width - 2 * padding)) / Math.max(1, xValues.length - 1);
    const scaleY = (yValue: number) => height - padding - (yValue * (height - 2 * padding)) / maxY;
    const path = points
    ...
    function onHitMove(event: MouseEvent) {
        try {
    ...
    gridHitArea.addEventListener('mousemove', onHitMove as EventListener);
    gridHitArea.addEventListener('mouseenter', onHitMove as EventListener);
    ...
    points.forEach((point, index) => {
        ...
        hitDot.addEventListener('mousemove', (event: Event) =>
            showGraphTooltip(
                `<strong>${escapeHtml(prettyTournamentName(point.tournament))}</strong><div>${(point.pct || 0).toFixed(1)}%</div>`,
        ...
        hitDot.addEventListener('mouseenter', (event: Event) =>
            showGraphTooltip(
                `<strong>${escapeHtml(prettyTournamentName(point.tournament))}</strong><div>${(point.pct || 0).toFixed(1)}%</div>`,
                (event as MouseEvent).clientX,
                (event as MouseEvent).clientY
 ...
 */
export function renderCopiesHistogram(container: HTMLElement, overall: OverallDistribution): void {
