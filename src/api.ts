 ...
 * @returns
 */
function fetchWithRetry<T>(url: string, operation: string, expectedType: string, fieldName?: string, options: FetchOptions = {}): Promise<T> {
 ...
 * @param options
 */
export async function fetchReportResource<T = any>(relativePath: string, operation: string, expectedType: string, fieldName: string, options: FetchOptions = {}): Promise<T> {
...
export function fetchTournamentsList(): Promise<string[]> {
    return fetchReportResource<string[]>('tournaments.json', 'tournaments list', 'array', 'tournaments list', { cache: true });
}
...
export async function fetchLimitlessTournaments(filters: LimitlessFilters = {}): Promise<LimitlessTournament[]> {
    ...
    const payload = await fetchWithRetry<LimitlessResponse>(url, 'Limitless tournaments', 'object', 'Limitless tournaments payload', {
        cache: true,
        cacheKey
    });
    ...
    const normalized = payload.data.map(normalizeLimitlessTournament).filter((t): t is LimitlessTournament => Boolean(t));
...
export async function fetchOverrides(): Promise<Record<string, string>> {
    try {
        ...
        const data = await fetchWithRetry<Record<string, string>>(url, 'thumbnail overrides', 'object', 'thumbnail overrides', {
            cache: true,
...
export async function fetchArchetypesList(tournament: string): Promise<ArchetypeIndexEntry[]> {
    ...
    return result.map(normalizeArchetypeIndexEntry).filter((e): e is ArchetypeIndexEntry => Boolean(e));
}
 ...
 * @returns
 */
async function resolveCardPricingEntry(cardId: string, requiredField: string | null, logLabel: string): Promise<any | null> {
