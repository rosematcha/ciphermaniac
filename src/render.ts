...
function escapeHtml(str: string): string {
    ...
    return String(str).replace(/[&<>"]/g, (ch: string) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[ch as keyof typeof map] as string);
}
 ...
 */
export function renderSummary(container: HTMLElement | null, deckTotal: number, count: number, visibleRows: number | null = null, totalRows: number | null = null) {
    if (!container) {
        return;
...
function expandGridRows(items: any[], overrides: Record<string, string>, targetTotalRows: number, options: RenderOptions = {}) {
    const grid = getGridElement();
...
function setupCardImage(img: HTMLImageElement | null, cardName: string, useSm: boolean, overrides: Record<string, string>, cardData: any) {
    if (!img) return;
...
function makeCardElement(cardData: any, useSm: boolean, overrides: Record<string, string>, renderFlags: RenderOptions = {}, previousCardIds: Set<string> | null = null): HTMLElement {
