...
interface BinderMeta {
  totalDecks: number;
  allDecks: number;
  tournaments: string[];
  archetypeStats: BinderArchetypeStat[];
}
...
interface BinderSections {
  aceSpecs: BinderCard[];
  staplePokemon: BinderCard[];
  archetypePokemon: ArchetypeGroup[];
  frequentSupporters: BinderCard[];
  nicheSupporters: BinderCard[];
  stadiums: BinderCard[];
  tools: BinderCard[];
  frequentItems: BinderCard[];
  nicheItems: BinderCard[];
  specialEnergy: BinderCard[];
  basicEnergy: BinderCard[];
}

interface AnalyzedEvents {
  events: Array<{ tournament: string; decks: Array<DeckRecord & { canonicalArchetype?: string }> }>;
...
const sortByPriority = (first: BinderCard, second: BinderCard) => {
 ...
 */
export function buildBinderDataset(
  analysis: AnalyzedEvents,
  includedArchetypes: Set<string> | null | undefined
): BinderDataset {
