...
export function createMultiSelectDropdown(state: AppState, config: DropdownConfig): DropdownInstance | null {
    ...
    const search = config.searchId
        ? (document.getElementById(config.searchId) as HTMLInputElement | null)
        : null;
    const chipsContainer = config.chipsId
        ? (document.getElementById(config.chipsId) as HTMLElement | null)
        : null;
            ...
            if (!silent && typeof config.onChange === 'function') {
                try {
                    ...
                    if (result && typeof (result as Promise<any>).catch === 'function') {
                        (result as Promise<any>).catch(error => logger.error(`Dropdown ${config.key} change handler rejected`, error));
                    }
                } catch (error) {
