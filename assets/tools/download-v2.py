...
def get_soup(url, session):
    ...
    return BeautifulSoup(resp.text, 'html.parser'), resp.headers
...
def download_thumbnails(all_decks, base_path, session):
    ...
    for card in unique_cards.values():
        ...
        if base_filename not in processed_filenames:
            ...
            for size in sizes_to_download:
                ...
                if not full_url:
                    missing_thumbs.append({"name": name, "set": set_code, "number": number, "size": size, "url_tried": full_url})
                    continue
                ...
                try:
                    ...
                    if resp and resp.status_code == 200:
                        ...
                        raise requests.exceptions.RequestException(f"Status {getattr(resp, 'status_code', 'ERR')}")
                except requests.exceptions.RequestException as e:
                    print(f"    - Could not download size {size}: {e}")
                    missing_thumbs.append({"name": name, "set": set_code, "number": number, "size": size, "url_tried": full_url})
...
def main(args):
    ...
    soup, headers = get_soup(args.url, session)
    if not soup: return
    ...
    with open(os.path.join(archetype_report_path, "index.json"), 'w') as f: json.dump(sorted(archetype_index_list), f, indent=2)
    print("Archetype index saved.")

    # Download thumbnails
    if not args.skip_thumbs:
        missing_thumbs = download_thumbnails(all_decks, ".")
        if missing_thumbs:
...
if __name__ == "__main__":
    ...
    parser.add_argument("url", help="The URL of the Limitless TCG tournament decklists page.")
    ...
    # Correct the URL if the base tournament page is given
    if args.url and not args.url.endswith('/decklists'):
