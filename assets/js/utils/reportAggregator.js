...
function buildAggregationKey(item) {
  ...
  const numberPart = typeof item.number === 'string' || typeof item.number === 'number'
  ...
  for (const aggregate of map.values()) {
    ...
    if (aggregate.distBuckets.size > 0) {
      dist = Array.from(aggregate.distBuckets.values())
        .map(bucket => {
          const percent = totalDecks > 0
            ? (bucket.players / totalDecks) * 100
            : 0;
...
export function aggregateReports(reports) {
  ...
  for (const report of reports) {
    ...
    for (const item of report.items) {
      ...
      if (!aggregate.uid && item.uid) { aggregate.uid = item.uid; }
      if (!aggregate.set && item.set) { aggregate.set = item.set; }
      if (!aggregate.number && (typeof item.number === 'string' || typeof item.number === 'number')) {
        aggregate.number = item.number;
      }
      if (!aggregate.category && item.category) { aggregate.category = item.category; }
      if (!aggregate.trainerType && item.trainerType) { aggregate.trainerType = item.trainerType; }
      if (!aggregate.energyType && item.energyType) { aggregate.energyType = item.energyType; }
      if (!aggregate.displayCategory && item.displayCategory) { aggregate.displayCategory = item.displayCategory; }
    }
  }

  const aggregatedItems = finalizeAggregates(aggregates, totalDecks)
    .sort((left, right) => (right.found ?? 0) - (left.found ?? 0));
