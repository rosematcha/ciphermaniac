...
import { fetchTournamentsList, fetchReport, getCardPrice } from '../api.js';
import { parseReport } from '../parse.js';
import { getCanonicalId, getDisplayName, getBaseName } from './identifiers.js';
import { ErrorBoundary, validators, logger } from '../utils/errorHandler.js';

/**
 * Find a card in items array by identifier
 * @param {Array} items - Array of card items
 * @param {string} cardIdentifier - Card identifier to search for
 * @returns {object|null} Found card item or null
 */
 ...
 * @param {string} cardIdentifier - Base card identifier
 * @returns {Promise<Array>} Array of card variants
 */
export async function collectCardVariants(cardIdentifier) {
  let validatedIdentifier, searchBaseName;
 ...
 * @param {string} cardIdentifier - Card identifier
...
export async function renderCardPrice(cardIdentifier) {
  ...
  try {
    ...
    await errorBoundary.execute(
      ...
      null,
      { loadingMessage: 'Loading price...', retryAttempts: 1 }
    );
  } catch (error) {
 ...
 * @param {string} cardIdentifier - Validated card identifier
 * @param {HTMLElement} container - Price container element
 * @returns {Promise<void>}
 */
