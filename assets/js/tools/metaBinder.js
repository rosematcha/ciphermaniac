import { fetchTournamentsList, fetchDecks, fetchOverrides, getCardPrice } from '../api.js';
...
function updateStats() {
  ...
  const coverageSelected = metrics
    ...
    : (selectionDecks ? Math.min(1, binderDecks / selectionDecks) : 0);
  const coverageMeta = metrics
    ? metrics.coverageMeta
    : (metaDecks ? Math.min(1, binderDecks / metaDecks) : 0);
...
function renderArchetypeControls() {
  ...
  for (const archetype of archetypes) {
    ...
    checkbox.checked = state.selectedArchetypes.size === 0
      ? true
    ...
    const caption = document.createElement('span');
    caption.innerHTML = `<strong>${archetype.displayName}</strong> ` +
